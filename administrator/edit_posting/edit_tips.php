...
<?php
        $id = $_GET['id'];
        $sql = mysqli_query($connection, "select * from story where story_id='$id'") or die(mysqli_error());
...
<h4><i class=icon-reorder></i>Edit Story</h4>
<span class=tools>
...
<div class=controls>
<input type=text placeholder="Masukan Judul" class=input-xlarge name=judul value="<?php echo $data['judul'] ?>" required/>
</div>
</div>
<div class=control-group>
...
<textarea cols=60 rows=10 id=posting name=isi required/><?php echo $data['isi'] ?></textarea>
</div>
</div>
</div>
<div class=control-group>
...
<div class="fileupload-new thumbnail" style=width:200px;height:150px><img src=../gambar/Story/<?php echo $data['gambar'] ?> alt /></div>
